...
import { Component, ViewChild, OnInit, OnDestroy, ViewChildren, QueryList, AfterViewInit, Input } from '@angular/core';
...
export class CcComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit() {
    if(!this.isStudentUserCard) {
      this.pageTitleService.setTitle('CC');
    };
  ...
  getMailbox() {
    ...
    if (this.studentId) {
      this.studentService.GetOneStudent(this.studentId).subscribe((resp) => {
        ...
        this.getMainMailbox(pagination, this.sortValue, type, new_mail, filter, recipientRank, this.user_id);
      });
    } else if (this.userId) {
  ...
  getMainMailbox(pagination, sortValue, type, new_mail, filter, recipientRank, user_id?) {
    const user = user_id ? user_id : null;
    this.subs.sink = this.mailboxService
      .getMainMail(pagination, sortValue, type, new_mail, filter, recipientRank, user)

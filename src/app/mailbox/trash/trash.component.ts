...
export class TrashComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit() {
    if(!this.isStudentUserCard) {
      this.pageTitleService.setTitle('Trash');
    };
  ...
  public openDialog(data) {
    ...
    swal
      ...
      .then((result) => {
        clearTimeout(this.timeOutVal);
        if (result.value) {
          this.subs.sink = this.mailboxService.deleteMail(mailId).subscribe((dataa: any) => {
            swal.fire({
              type: 'success',
  ...
  onDeleteMail() {
    ...
    swal
      ...
      .then((result) => {
        ...
        if (result.value) {
          this.viewMessageData = [];
          this.subs.sink = this.mailboxService.deleteMultipleMail(this.mailSelected).subscribe((data: any) => {
            swal.fire({
              type: 'info',
              title: this.translate.instant('MailBox.MESSAGES.DELMSG'),
              text: '',
  ...
  mailMoveTo(mail_type) {
    ...
    this.viewMessageData = [];
    this.subs.sink = this.mailboxService.updateMultipleMail(this.mailSelected, dataPost).subscribe((data: any) => {
      if (!this.isReset) {
        this.getTrashMAilbox();
      }
    });

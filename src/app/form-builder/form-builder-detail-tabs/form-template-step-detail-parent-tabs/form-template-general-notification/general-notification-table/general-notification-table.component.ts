...
@Component({
  selector: 'ms-general-notification-table',
  templateUrl: './general-notification-table.component.html',
  styleUrls: ['./general-notification-table.component.scss']
})
export class GeneralNotificationTableComponent implements OnInit {
  ...
  getStepNotificationAndMessages() {
    ...
    // step id null because general notification and new parameter is_for_general
    const generalNotification = {
      stepID: null,
      is_for_general: true
    };
    this.subs.sink = this.formBuilderService.getAllStepNotificationsAndMessages(this.templateId, generalNotification.stepID, pagination, generalNotification.is_for_general).subscribe(    
      (resp) => {
        if (resp && resp.length) {

...
import { ValidateStudentCvPresentationComponent } from 'app/school/school-student-table/validate-student-cv-presentation/validate-student-cv-presentation.component';
import { ValidateStudentDocExpectedComponent } from 'app/school/school-student-table/validate-student-doc-expected/validate-student-doc-expected.component';
...
@Component({
  selector: 'ms-title-manager-student-table',
  templateUrl: './title-manager-student-table.component.html',
  styleUrls: ['./title-manager-student-table.component.scss'],
  providers: [ParseStringDatePipe, ParseLocalToUtcPipe, ParseUtcToLocalPipe],
})
export class TitleManagerStudentTableComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    this.subs.sink = this.paginator.page
      .pipe(
        startWith(null),
        tap(() => {
          if (this.selectedRncpTitleId && this.selectedClassId && !this.isReset) {
            this.getDataFromParam();
          }
  ...
  getDataFromParam() {
    ...
    const studentsGet = this.studentService.getStudentsTitleManager(
  ...
  cleanFilterData() {
    ...
    Object.keys(filterData).forEach((key) => {
      ...
      if (filterData[key]) {
        if (key === 'full_name' || key === 'school_id') {
  ...
  getTableWidth() {
    ...
    const staticColumnWidth = 250 + 200 + 60 + 75 + +60 * 6 + 80 + 190;
  ...
  getSchoolDropdownList() {
    ...
    this.subs.sink = this.schoolService.getAllSchoolAsDropdownList(schoolType, this.selectedRncpTitleId, this.selectedClassId).subscribe((response) => {
      if(response) {
      ...
      map((searchTxt) =>
        this.schoolFilterList
          ...
          .sort((firstData, secondData) => {
            if (
              this.utilityService.simpleDiacriticSensitiveRegex(this.utilityService.disregardSpace(firstData.short_name)).toLowerCase() <
              this.utilityService.simpleDiacriticSensitiveRegex(this.utilityService.disregardSpace(secondData.short_name)).toLowerCase()
            ) {
              return -1;
            } else if (
              this.utilityService.simpleDiacriticSensitiveRegex(this.utilityService.disregardSpace(firstData.short_name)).toLowerCase() >
  ...
  goToComentaries(data) {
    this.router.navigate(['/school', data.school._id], {
      queryParams: {
        title: data.rncp_title._id,
        class: data.current_class._id,
        student: data._id,
        open: 'student-cards',
        selectedTab: 'Commentaries',
      },
    });
  }
  ...
  showButtonConnectAs() {
    return this.permissionService.showActionsConnectAsStudentSubmenuPerm();
  }

  showButtonTowardAdmis() {
    return this.permissionService.showActionsAdministrationStudentSubmenuPerm();
  }

  showButtonResignation() {
    return this.permissionService.showActionsResignationStudentSubmenuPerm();
  }

  showButtonStudentFile() {
    return this.permissionService.showActionsStudentFileStudentSubmenuPerm();
  }

  showButtonSendEmailStudent() {
    return this.permissionService.showActionsSendEmailStudentSubmenuPerm();
  }

  showButtonCommentaries() {
    return this.permissionService.showActionsCommentariesStudentSubmenuPerm();
  }

  showButtonSendEmailAcadir() {
    return this.permissionService.showActionsEmailAcadirStudentSubmenuPerm();
  }
}

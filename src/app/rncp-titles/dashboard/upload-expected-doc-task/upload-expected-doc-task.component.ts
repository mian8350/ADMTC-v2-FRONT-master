...
@Component({
  selector: 'ms-upload-expected-doc-task',
  templateUrl: './upload-expected-doc-task.component.html',
  styleUrls: ['./upload-expected-doc-task.component.scss'],
})
export class UploadExpectedDocTaskComponent implements OnInit, OnDestroy {
  ...
  docper = ['pdf'];
  default = ['doc', 'docx', 'ppt', 'pptx', 'txt', 'pdf', 'xlsx', 'xls'];
  ...
  constructor(
    ...
    public dialogref: MatDialogRef<UploadExpectedDocTaskComponent>,
  ...
  ngOnInit() {
    if (this.selectedTask) {
      const isStudent =
        ...
        this.selectedTask.user_selection.user_type_id._id === this.studentId;
      this.sectionSelected = isStudent ? true : false;
    }
    ...
    if (this.selectedTask && this.selectedTask.expected_document && this.selectedTask.expected_document.file_type) {
      ...
      this.fileTypesControl.patchValue('default');
  ...
  populateAllowedFileList() {
    if (this.selectedFileType) {
      ...
      switch (this.selectedFileType) {
        case 'default':
          this.default.forEach((extension, index) => {
  ...
  generateDocName() {
    if (this.selectedTask.for_each_student) {
      ...
      this.docName = this.selectedTask.description;
    }
  }
  ...
  populateUploadDocForm() {
    ...
    if (this.sectionSelected) {
      ...
      this.industryCtrl.setValue('other');
      this.uploadDocForm.get('document_industry').patchValue('other');
    }
  }
  ...
  uploadFile() {
    ...
    if (selectedFileSizeInGb < 1) {
      ...
      //   },
      //   (err) => {
      //     // handle when there is error in graphql
      //     this.isWaitingForResponse = false;
      //     this.uploadFailSwal();
      //   },
      // );
  ...
  uploadDocExpected() {
    ...
    this.testCorrectionService
      .SingleUploadDocumentExpected(
        this.selectedFile,
        this.uploadDocForm.get('document_name').value,
        this.uploadDocForm.get('document_title').value,
        this.uploadDocForm.get('document_industry').value,
  ...
  createAcadDoc() {
    ...
    this.subs.sink = this.acadKitService.createAcadDoc(payload).subscribe(
      (resp) => {
        ...
        this.uploadedDocId = resp._id;
        this.addDocToFolder06(resp._id);
      },
      (err) => {
        this.swalError(err);
      },
  ...
  createUpdateCorrection(resp: TestCorrection[]) {
    ...
    let expectedDocs: ExpectedDocPayload[] = [];
    let elementOfProofDoc = '';
    ...
    resp.forEach((testCorrection) => {
      if (this.selectedTask.for_each_student) {
        if (testCorrection.student && testCorrection.student._id === this.selectedTask.student_id._id) {
          ...
          elementOfProofDoc = testCorrection.element_of_proof_doc ? testCorrection.element_of_proof_doc._id : '';
        }
      } else if (testCorrection.student && testCorrection.student._id === this.selectedTask.user_selection.user_id.student_id._id) {
        testCorrectionId = testCorrection._id;
        expectedDocs = this.getDocExpected(testCorrection);
        elementOfProofDoc = testCorrection.element_of_proof_doc ? testCorrection.element_of_proof_doc._id : '';
      }
    ...
    const testCorrectionPayload = {
      ...
      student: this.selectedTask.for_each_student
        ? this.selectedTask.student_id._id
        : this.selectedTask.user_selection.user_id.student_id._id,
      element_of_proof_doc: elementOfProofDoc,
    };
    if (!elementOfProofDoc) {
      delete testCorrectionPayload.element_of_proof_doc;
    }

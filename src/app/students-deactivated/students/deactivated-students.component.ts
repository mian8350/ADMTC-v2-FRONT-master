...
@Component({
  selector: 'ms-deactivated-students',
  templateUrl: './deactivated-students.component.html',
  styleUrls: ['./deactivated-students.component.scss'],
  providers: [ParseStringDatePipe],
})
export class DeactivatedStudentsComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit() {
    this.pageTitleService.setTitle('List of Deactivated Students');
    ...
    this.admissionStatusSuperFilterList = [
      { key: 'deactivated', value: this.translate.instant('deactivated') }
    ];
    ...
    if (!!this.permissions.getPermission('Chief Group Academic')) {
      ...
      this.subs.sink = this.authService.getUserById(this.currentUser._id).subscribe((res) => {
        ...
        this.subs.sink = this.studentService
          .getAllStudentsChiefGroupDeactivated(pagination, this.school_ids, this.sortValue, filter)
      ...
      this.subs.sink = this.studentService.getAllStudentsCRDeactivated(pagination, this.sortValue, filter).subscribe((students: any) => {
        if (students && students.length) {
          this.dataSource.data = students;
          this.paginator.length = students[0].count_document;
          this.dataCount = students[0].count_document;
      ...
      this.subs.sink = this.studentService
        .getAllStudentsMentorDeactivated(pagination, mentor_id, this.sortValue, filter)
        .subscribe((students: any) => {
          if (students && students.length) {
            this.dataSource.data = students;
            this.paginator.length = students[0].count_document;
      ...
      this.subs.sink = this.studentService.getAllStudentDeactivated(pagination, this.sortValue, filter).subscribe((students: any) => {
        ...
        if (resp && resp.user) {
          ...
          Swal.fire({
            ...
            if (tempUser.entities[0].type._id === studentType) {
              this.authService.connectAsStudent(resp,tempUser.entities[0].type.name,'ifrStudent')
            } else {
  ...
  editStudent(student) {
    ...
    // });
    window.open(
      `./school/${student.school._id}?title=${student.rncp_title._id}&class=${student.current_class._id}&student=${student._id}&studentStatus=deactivated&open=student-cards`,
      '_blank',
    );
  }
  ...
  getAllStudentExportData(pageNumber: number) {
    ...
    if (!!this.permissions.getPermission('Chief Group Academic')) {
      ...
      filter = this.cleanFilterData();
      this.subs.sink = this.studentService
        .getAllStudentsCRDeactivated(pagination, this.sortValue, filter)
        .subscribe((students: any) => {
          if (students && students.length) {
            this.allStudentForExport.push(...students);
            const page = pageNumber + 1;
      ...
      this.subs.sink = this.studentService
        .getAllStudentDeactivated(pagination, this.sortValue, filter)
        .subscribe((students: any) => {
  ...
  exportAllData(dataa) {
    ...
    if (exportData && exportData.length) {
      exportData.forEach((item) => {
        ...
        obj[6] = item.first_name;
        obj[7] = item.last_name;
        // obj[7] = '=HYPERLINK("' + `http://www.admtc.pro/school/${schoolId}?title=${titleId}&class=${classId}&student=${item._id}&open=student-cards&selectedTab=Identity&selectedSubTab=Identity` + '"; "' + this.translate.instant('LINK') + '")';
        obj[8] = '-';
        obj[9] = item.createdAt ? moment(parseInt(item.createdAt)).format('DD/MM/YYYY') : '-';
        obj[10] = datebirth ? moment(datebirth, 'DD/MM/YYYY').format('DD/MM/YYYY') : '-';
        obj[11] = item.place_of_birth;

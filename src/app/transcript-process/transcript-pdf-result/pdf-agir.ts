...
export class PdfAGIR2020 {
  static computerRDCPDF(student: any, result, expertise, journalText?, pdfData?, studentTranscript?) {
    ...
    const cpeaPdf = `
                ...
                <b>Analyste en Génie Informatique et réseaux<b/> <br>
                <b>Certification Professionnelle de niveau 6 – Fiche RNCP29069<b/>
                </div>
  ...
  static getUserInfo(student, result, studentTranscript?) {
    return `
    ...
    if (result && result.block_of_competence_conditions && pdfData) {
      ...
      pdfData.block_of_competence_conditions.forEach((block, blockIndex) => {
        ...
        block.subjects.forEach((subject, subjectIndex) => {
          ...
          if (subjectIndex === block.subjects.length - 1) {
            subjectRow =
              subjectRow +
            ...
            subjectRow =
              subjectRow +
              `<tr style="height:10px">
                      <td colspan="4" style="border-bottom: 1px dotted #d3d3d3;"></tb>
                  </tr>`;
          }
      ...
      table =
        ...
        `</tbody>
        <tfoot>
          <tr style="float: unset;height: 10px !important;"></tr>
          <tr>
            <td colspan="4" class="header-card" style="float: unset;border: 1px solid #17365d;height: 25px !important;">
              Note Finale :  ${result.total_mark ? result.total_mark : 0}
            </td>
          </tr>
          <tr style="float: unset;height: 10px !important;"></tr>
          <tr>
  ...
  static getFooter(journalText) {
    ...
    <div style="width: 90%; margin: 5px auto 30px; text-align: center; position: fixed; bottom: 0; font-weight: 100">
      <div style="font-size:9px; display:block">
        <b>Certification Professionnelle - Analyste en Génie Informatique et Réseaux</b>
      </div>
      <div style="font-size:9px; display:block">
        ${journalText}
      </div>
      <div style="font-size:9px; display:block">
        INSTA - 17 Rue Linné, 75005 Paris
      </div>
    </div>`;
  }

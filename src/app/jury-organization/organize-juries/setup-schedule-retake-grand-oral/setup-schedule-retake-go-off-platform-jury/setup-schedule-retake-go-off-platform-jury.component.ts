...
@Component({
  selector: 'ms-setup-schedule-retake-go-off-platform-jury',
  templateUrl: './setup-schedule-retake-go-off-platform-jury.component.html',
  styleUrls: ['./setup-schedule-retake-go-off-platform-jury.component.scss'],
  providers: [ParseUtcToLocalPipe, ParseLocalToUtcPipe],
})
export class SetupScheduleRetakeGoOffPlatformJuryComponent implements OnInit {
  ...
  multiplePublish() {
    ...
    const studentIds = [...this.studentSelected];
    ...
    this.isWaitingForResponse = true;
      this.subs.sink = this.juryOrganizationService.getOneJuryOrganizationSchedule(element._id).subscribe((resp) => {
        ...
        if (resp && resp.blocks_for_grand_oral && resp.blocks_for_grand_oral.length) {
          ...
          isAllBlockExempted = tempBlocks.filter((block) => block.is_retaking).every(blockData => {
  ...
  masterToggle() {
    if (this.isAllSelected()) {
      ...
      this.studentSelected = [];
      this.dataSource.data.forEach((row) => this.studentSelected.push(row.student_id._id));
      this.studentDeselected = [];
  ...
  selectRow(event, row) {
    ...
    if (event.checked) {
      const studentIdDeselected = this.studentDeselected.filter((data) => data !== row);
      this.studentDeselected = studentIdDeselected;

      // -----
      this.studentSelected.push(row);

    } else {
      // clear deselected student from variable studentSelected
      const studentIdSelected = this.studentSelected.filter((data) => data !== row);
      this.studentSelected = studentIdSelected;

      // -----
      this.studentDeselected.push(row);

    }
  }

  isRowSelected(studentId) {
    const found = this.studentSelected?.find((row) => row === studentId);
    if (found) {
      return true;
    } else {
      return false;
